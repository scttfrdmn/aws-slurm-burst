...
func TestASBADecision_JSON(t *testing.T) {
	decision := &ASBADecision{
		...
		PerformanceModel: PerformanceModel{
			OnPremiseWaitTime: Duration(45 * time.Minute),
			AWSProvisionTime:  Duration(5 * time.Minute),
			NetworkLatency:    Duration(2 * time.Millisecond),
			StorageLatency:    Duration(10 * time.Millisecond),
		},
		Confidence:      0.85,
